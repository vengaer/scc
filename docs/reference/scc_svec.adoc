...
`scc_svec` provides a generic "small" vector particularly suitable for working with smaller sets of elements. The initial structure is
...
Expands to an opaque, fat pointer suitable for representing an `scc_svec` containing elements of type `type`. Once initialized,
it holds the address of the base of the vector and may be dereferenced and indexed as any raw pointer. The behavior when
accessing elements beyond the end of the vector is undefined. The size may be queried using `scc_svec_size`. The pointer
should not be modified through any means other than what is provided by the `scc_svec` API.
...
=== void scc_svec_erase(void *svec, size_t index)

Erase the `index`'th element in the vector. No bounds checking is performed. Elements at indices greater than `index` (if any) are copied
down one step.

.Erasing the 0th element in a vector
[source]
----
...
=== void scc_svec_erase_range(void *svec, size_t first, size_t end)

Erase the elements at index range `[first,end)`. Elements in the range `[end,scc_svec_size(svec))` are copied to replace
the erased range. `first` must be an index in range `[0,scc_svec_size(svec))`. `end` must be in range `[0,scc_svec_size(svec)]`,
i.e. it may be the offset of the first element _after_ the vector.

If `first <= end`, the function returns immediately. No other bounds checking is performed.
...
=== type scc_svec_at(void *svec, size_t index)
...
== Convenience Macros
=== scc_svec_foreach(void *iter, void *svec)

Produces a scope executed `scc_svec_size(vec)` times with `iter` referring to each element in the vector, in order.

.Printing a vector of integers
[source]
----
include::snips/scc_svec_foreach.csnip[tags=scc_svec_foreach]
----
...
=== scc_svec_foreach_reversed(void *iter, void *svec)

Produces a scope executed `scc_svec_size(svec)` times with `iter` referring to each element in the vector in reverse order.

.Printing a vector of doubles in reverse
[source]
----
include::snips/scc_svec_foreach_reversed.csnip[tags=scc_svec_foreach_reversed]
----
...
=== scc_svec_foreach_by(void *iter, void *svec, size_t by)

Iterates over `svec` with `iter` referring to every `by` element.

.Printing every other element in a vector of integers
[source]
----
include::snips/scc_svec_foreach_by.csnip[tags=scc_svec_foreach_by]
----
...
=== scc_svec_foreach_reversed_by(void *iter, void *svec, size_t by)

Iterator over `svec` in reverse order with `iter` referring to every `by` element.

.Printing every sixth element in a vector in reverse order
[source]
----
include::snips/scc_svec_foreach_reversed_by.csnip[tags=scc_svec_foreach_reversed_by]
----
