...
`scc_vec` is a generic, dynamically allocated array. Individual elements of the vector may be accessed using `[]`.

Note that `scc_svec` should generally preferred over `scc_svec`. It reference is found link:scc_svec.html[here].
...
Expands to an opaque, fat pointer suitable for representing an `scc_vec` containing elements of type `type`. Once initialized,
it holds the address of the base of the vector and may be dereferenced and indexed as any raw pointer. The behavior when
accessing elements beyond the end of the vector is undefined. The size may be queried using `scc_vec_size`. The pointer
should not be modified through any means other than what is provided by the `scc_vec` API.
...
Initialize an `scc_vec` for storing instances of `type` initialized with `args`. At least one argument must be passed in `args`.
...
=== void scc_vec_erase(void *vec, size_t iter)
...
=== void scc_vec_erase_range(void *vec, size_t first, size_t end)

Erase the elements at index range `[first,end)`. Elements in the range `[end,scc_svec_size(svec))` are copied to replace
the erased range. `first` must be an index in range `[0,scc_vec_size(vec))`. `end` must be in range `[0,scc_vec_size(vec)]`,
...
.Erasing 5 elements at offset 3
----
