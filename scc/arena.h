...
#define scc_arena_impl_baseoff(type) sizeof(scc_chunk_impl_layout(type))

#define scc_arena_new(type)                                                                        \
    (struct scc_arena)                                                                             \
    {                                                                                              \
        .ar_baseoff = scc_arena_impl_baseoff(type), .ar_elemsize = sizeof(type),                   \
...
#define scc_arena_clone(arena)                                                                     \
    (struct scc_arena)                                                                             \
    {                                                                                              \
        .ar_baseoff = (arena)->ar_baseoff, .ar_elemsize = (arena)->ar_elemsize,                    \
...
#define scc_arena_foreach_chunk_safe(iter, hare, arena)                                            \
    for (iter = arena->ar_first, hare = iter ? iter->ch_next : 0; iter;                            \
         iter = hare, hare = hare ? hare->ch_next : 0)

#define scc_arena_foreach_chunk_lagging(iter, tortoise, arena)                                     \
    for (iter = arena->ar_first, tortoise = 0; iter; tortoise = iter, iter = iter->ch_next)
