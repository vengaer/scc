...
#define scc_container(addr, type, member) scc_container_qual(addr, type, member, )
...
#define scc_arrsize(addr) (sizeof(addr) / sizeof((addr)[0]))

#define scc_align(addr, bound) (((addr) + (bound) - 1u) & ~((bound) - 1u))
...
#define scc_memmove(dst, src, n) (assert((n) > 0u), memmove((dst), (src), (n)))

#define scc_memcpy(dst, src, n) (assert((n) > 0u), memcpy((dst), (src), (n)))
