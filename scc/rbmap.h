...
#define scc_rbmap(keytype, valuetype) scc_rbmap_impl_pair(keytype, valuetype) *
...
#define scc_rbmap_new_dyn(keytype, valuetype, compare)                                             \
    scc_rbtree_impl_new_dyn(sizeof(scc_rbmap_impl_layout(keytype, valuetype)),                     \
                            &scc_arena_new(scc_rbmnode_impl_layout(keytype, valuetype)), compare,  \
                            scc_rbmap_impl_curroff(keytype, valuetype),                            \
...
#define scc_rbmap_insert(mapaddr, key, value)                                                      \
    scc_rbmap_impl_insert(                                                                         \
        ((*(mapaddr))->rm_key = (key), (*(mapaddr))->rm_value = (value), (mapaddr)),               \
        sizeof(**(mapaddr)),                                                                       \
...
#define scc_rbmap_find(map, key)                                                                   \
    scc_rbmap_impl_find(((map)->rm_key = (key), (map)), ((unsigned char const *)&(map)->rm_value - \
                                                         (unsigned char const *)&(map)->rm_key))

#define scc_rbmap_remove(map, key)                                                                 \
    scc_rbtree_impl_remove(((map)->rm_key = (key), (map)), sizeof(*(map)))
...
#define scc_rbmap_clone(map) scc_rbtree_impl_clone(map, sizeof(*(map)))

#define scc_rbmap_foreach(iter, map)                                                               \
    for (void const *scc_pp_cat_expand(scc_rbmap_end_, __LINE__) =                                 \
             ((iter) = scc_rbmap_impl_leftmost_pair(map), scc_rbmap_impl_iterstop(map));           \
...
#define scc_rbmap_foreach_reversed(iter, rbmap)                                                    \
    for (void const *scc_pp_cat_expand(scc_rbmap_end_, __LINE__) =                                 \
             (iter = scc_rbmap_impl_rightmost_pair(rbmap), scc_rbmap_impl_iterstop(rbmap));        \
         iter != scc_pp_cat_expand(scc_rbmap_end_, __LINE__);                                      \
