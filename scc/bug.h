...
#else
#define scc_bug_on(expr) assert(!(expr))
#endif

#define scc_static_assert(...)                                                                     \
    scc_pp_cat_expand(scc_static_assert_cond_only_,                                                \
                      scc_assert_match(scc_assert_count(__VA_ARGS__), 1))(__VA_ARGS__)

#define scc_assert_match(token, match)                                                             \
    scc_assert_chk(scc_pp_expand(scc_pp_cat_expand(                                                \
        scc_pp_cat_expand(scc_pp_cat_expand(scc_assert_probe_, token), _), match))())
...
#define scc_static_assert_cond_only_1(cond) (void)((unsigned char[(!!(cond) << 1) - 1]){0})

#define scc_static_assert_cond_only_0(cond, msg) scc_static_assert_cond_only_1(cond)
