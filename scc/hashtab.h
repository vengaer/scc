...
#ifdef SCC_PERFEVTS
#define SCC_HASHTAB_INJECT_PERFEVTS(name) struct scc_hashtab_perfevts name;
#else
...
#define scc_hashtab_with_hash(type, eq, hash)                                                      \
    (type *)scc_hashtab_impl_new(                                                                  \
        (void *)&(scc_hashtab_impl_layout(type)){                                                  \
            .ht1 =                                                                                 \
                {                                                                                  \
                    .ht0 = {.ht_eq = eq, .ht_hash = hash, .ht_capacity = SCC_HASHTAB_STACKCAP},    \
                },                                                                                 \
        },                                                                                         \
        scc_hashtab_impl_curroff(type), scc_hashtab_impl_metaoff(type))

#define scc_hashtab_with_hash_dyn(type, eq, hash)                                                  \
    (type *)scc_hashtab_impl_new_dyn(                                                              \
        eq, hash, SCC_HASHTAB_STACKCAP, sizeof(scc_hashtab_impl_layout(type)),                     \
        scc_hashtab_impl_curroff(type), scc_hashtab_impl_metaoff(type))

#define scc_hashtab_new(type, eq) scc_hashtab_with_hash(type, eq, scc_hash_fnv1a)

#define scc_hashtab_new_dyn(type, eq) scc_hashtab_with_hash_dyn(type, eq, scc_hash_fnv1a)
...
#define scc_hashtab_impl_base_qual(tab, qual)                                                      \
    scc_container_qual((unsigned char qual *)(tab) - scc_hashtab_impl_bkpad(tab),                  \
                       struct scc_hashtab_base, ht_fwoff, qual)

#define scc_hashtab_impl_base(tab) scc_hashtab_impl_base_qual(tab, )
...
#define scc_hashtab_foreach(iter, tab)                                                             \
    for ((iter) = scc_hashtab_impl_iter_begin(tab, sizeof(*(tab))); (iter);                        \
