...
#define scc_vec_impl_base_qual(vec, qual)                                                          \
    scc_container_qual((unsigned char qual *)(vec) - scc_vec_impl_npad(vec), struct scc_vec_base,  \
                       sv_buffer, qual)

#define scc_vec_impl_base(vec) scc_vec_impl_base_qual(vec, )
...
#define scc_vec_new(type)                                                                          \
    (type *)scc_vec_impl_new((void *)&(scc_vec_impl_layout(type)){0}, scc_vec_impl_offset(type),   \
                             SCC_VEC_STATIC_CAPACITY)

#define scc_vec_new_dyn(type)                                                                      \
    (type *)scc_vec_impl_new_dyn(sizeof(scc_vec_impl_layout(type)), scc_vec_impl_offset(type),     \
                                 SCC_VEC_STATIC_CAPACITY)

void *scc_vec_impl_from(void *restrict vec, void const *restrict data, size_t size,
                        size_t elemsize);

void *scc_vec_impl_from_dyn(size_t basecap, size_t offset, void const *data, size_t size,
                            size_t elemsize);

#define scc_vec_from(type, ...)                                                                    \
    scc_vec_impl_from(scc_vec_new(type), (type[]){__VA_ARGS__},                                    \
                      scc_arrsize(((type[]){__VA_ARGS__})), sizeof(type))

#define scc_vec_from_dyn(type, ...)                                                                \
    scc_vec_impl_from_dyn(SCC_VEC_STATIC_CAPACITY, scc_vec_impl_offset(type),                      \
                          (type[]){__VA_ARGS__}, scc_arrsize(((type[]){__VA_ARGS__})),             \
                          sizeof(type))

_Bool scc_vec_impl_resize(void *vecaddr, size_t size, size_t elemsize);

#define scc_vec_resize(vecaddr, size) scc_vec_impl_resize(vecaddr, size, sizeof(**(vecaddr)))

void scc_vec_impl_erase(void *vec, size_t index, size_t elemsize);

#define scc_vec_erase(vec, index) scc_vec_impl_erase(vec, index, sizeof(*(vec)))
...
#define scc_vec_pop(vec) ((vec)[--scc_vec_impl_base(vec)->sv_size])
...
#define scc_vec_clone(vec) scc_vec_impl_clone(vec, sizeof(*(vec)))

#define scc_vec_foreach(iter, vec) scc_vec_foreach_by(iter, vec, 1)

#define scc_vec_foreach_reversed(iter, vec) scc_vec_foreach_reversed_by(iter, vec, 1)

#define scc_vec_foreach_by(iter, vec, by)                                                          \
    for (void const *scc_pp_cat_expand(scc_vec_end, __LINE__) =                                    \
             ((iter) = (vec), &(vec)[scc_vec_size(vec)]);                                          \
         (void const *)(iter) < scc_pp_cat_expand(scc_vec_end, __LINE__); (iter) += (by))
