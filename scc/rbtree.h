...
#define scc_rbnode_link_qual(node, idx, qual)                                                      \
    (*(struct scc_rbnode_base * qual *)((unsigned char qual *)&(node)->rn_left +                   \
                                        (scc_rbnode_link_offset(node)) * (idx)))

#define scc_rbnode_link(node, idx) scc_rbnode_link_qual(node, idx, )

#define scc_rbnode_value_qual(base, node, qual)                                                    \
    ((void qual *)(((unsigned char qual *)node) + ((base)->rb_dataoff)))

#define scc_rbnode_value(base, node) scc_rbnode_value_qual(base, node, )
...
#define scc_rbtree_new_dyn(type, compare)                                                          \
    (type *)scc_rbtree_impl_new_dyn(sizeof(scc_rbtree_impl_layout(type)),                          \
                                    &scc_arena_new(scc_rbnode_impl_layout(type)), compare,         \
                                    scc_rbtree_impl_curroff(type), scc_rbnode_impl_valoff(type))
...
#define scc_rbnode_impl_base_qual(valaddr, qual)                                                   \
    scc_container_qual((unsigned char qual *)valaddr - scc_rbnode_bkoff(valaddr),                  \
                       struct scc_rbnode_base, rn_data, qual)

#define scc_rbnode_impl_base(valaddr) scc_rbnode_impl_base_qual(valaddr, )

#define scc_rbtree_impl_base_qual(rbtree, qual)                                                    \
    scc_container_qual((unsigned char qual *)(rbtree) - scc_rbtree_impl_npad(rbtree),              \
                       struct scc_rbtree_base, rb_fwoff, qual)

#define scc_rbtree_impl_base(rbtree) scc_rbtree_impl_base_qual(rbtree, )
...
#define scc_rbtree_find(rbtree, value) scc_rbtree_impl_find((*(rbtree) = (value), (rbtree)))
...
#define scc_rbtree_clone(rbtree) scc_rbtree_impl_clone(rbtree, sizeof(*(rbtree)))

#define scc_rbtree_foreach(iter, rbtree)                                                           \
    for (void const *scc_pp_cat_expand(scc_rbtree_end_, __LINE__) =                                \
             (iter = scc_rbtree_impl_leftmost_value(rbtree), scc_rbtree_impl_iterstop(rbtree));    \
...
#define scc_rbtree_foreach_reversed(iter, rbtree)                                                  \
    for (void const *scc_pp_cat_expand(scc_rbtree_end_, __LINE__) =                                \
             (iter = scc_rbtree_impl_rightmost_value(rbtree), scc_rbtree_impl_iterstop(rbtree));   \
         iter != scc_pp_cat_expand(scc_rbtree_end_, __LINE__);                                     \
