...
#define scc_btmap(keytype, valuetype) scc_btmap_impl_pair(keytype, valuetype) *
...
#define scc_btmap_with_order(keytype, valuetype, compare, order)                                   \
    scc_btmap_impl_with_order(                                                                     \
        &(scc_btmap_impl_layout(keytype, valuetype, order)){                                       \
            .btm1 =                                                                                \
                {                                                                                  \
                    .btm0 = {.btm_order = order,                                                   \
                             ...
                             .btm_arena = scc_arena_new(                                           \
                                 scc_btmnode_impl_layout(keytype, valuetype, order)),              \
                             .btm_compare = compare,                                               \
...
#define scc_btmap_new(keytype, valuetype, compare)                                                 \
    scc_btmap_impl_new(                                                                            \
        &(scc_btmap_impl_layout(keytype, valuetype, SCC_BTMAP_DEFAULT_ORDER)){                     \
            .btm1 =                                                                                \
                {                                                                                  \
                    .btm0 = {.btm_order = SCC_BTMAP_DEFAULT_ORDER,                                 \
                             .btm_keyoff = scc_btmnode_impl_keyoff(keytype),                       \
                             .btm_valoff = scc_btmnode_impl_valoff(keytype, valuetype,             \
                                                                   SCC_BTMAP_DEFAULT_ORDER),       \
                             .btm_linkoff = scc_btmnode_impl_linkoff(keytype, valuetype,           \
                                                                     SCC_BTMAP_DEFAULT_ORDER),     \
                             .btm_keysize = sizeof(keytype),                                       \
                             .btm_valsize = sizeof(valuetype),                                     \
                             .btm_arena = scc_arena_new(scc_btmnode_impl_layout(                   \
                                 keytype, valuetype, SCC_BTMAP_DEFAULT_ORDER)),                    \
                             .btm_compare = compare,                                               \
                             .btm_kvoff = scc_btmap_impl_pair_valoff(keytype, valuetype)},         \
                },                                                                                 \
        },                                                                                         \
...
#define scc_btmap_impl_base_qual(btmap, qual)                                                      \
    scc_container_qual((unsigned char qual *)(btmap) - scc_btmap_impl_npad(btmap),                 \
                       struct scc_btmap_base, btm_fwoff, qual)

#define scc_btmap_impl_base(btmap) scc_btmap_impl_base_qual(btmap, )
...
#define scc_btmap_insert(btmapaddr, key, value)                                                    \
    scc_btmap_impl_insert(                                                                         \
        ((*(btmapaddr))->btm_key = (key), (*(btmapaddr))->btm_value = (value), (btmapaddr)))

void *scc_btmap_impl_find(void *btmap);

#define scc_btmap_find(btmap, key) scc_btmap_impl_find((((btmap)->btm_key = (key)), (btmap)))

_Bool scc_btmap_impl_remove(void *btmap);

#define scc_btmap_remove(btmap, key) scc_btmap_impl_remove((((btmap)->btm_key = (key)), (btmap)))
