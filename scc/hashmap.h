...
#define scc_hashmap(keytype, valuetype) scc_hashmap_impl_pair(keytype, valuetype) *
...
#ifdef SCC_PERFEVTS
#define SCC_HASHMAP_INJECT_PERFEVTS(name) struct scc_hashmap_perfevts name;
#else
...
#define scc_hashmap_with_hash(keytype, valuetype, eq, hash)                                        \
    scc_hashmap_impl_new(                                                                          \
                    ...
                    .hm1 =                                                                         \
                        {                                                                          \
                            .hm0 = {.hm_eq = eq,                                                   \
                                    ...
                                    .hm_pairsize =                                                 \
                                        sizeof(scc_hashmap_impl_pair(keytype, valuetype)),         \
        ...
        scc_hashmap_impl_pair_valoff(keytype, valuetype), sizeof(keytype))
...
#define scc_hashmap_impl_base_qual(map, qual)                                                      \
    scc_container_qual((unsigned char qual *)(map) - scc_hashmap_impl_bkpad(map),                  \
                       struct scc_hashmap_base, hm_fwoff, qual)

#define scc_hashmap_impl_base(map) scc_hashmap_impl_base_qual(map, )
...
#define scc_hashmap_insert(mapaddr, key, value)                                                    \
    scc_hashmap_impl_insert(                                                                       \
        ((*(mapaddr))->hp_key = (key), (*(mapaddr))->hp_val = (value), (mapaddr)),                 \
        sizeof((*(mapaddr))->hp_key), sizeof((*(mapaddr))->hp_val))

inline size_t scc_hashmap_capacity(void const *map)
{
    struct scc_hashmap_base const *base = scc_hashmap_impl_base_qual(map, const);
...
#define scc_hashmap_find(map, key)                                                                 \
    scc_hashmap_impl_find(((map)->hp_key = (key), (map)), sizeof((map)->hp_key),                   \
                          sizeof((map)->hp_val))
