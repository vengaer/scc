...
static inline void scc_btnode_merge_right(
    ...
    size_t nmov = scc_btnode_merge(base, sibling, node, p, bound, elemsize) + 1u; /* NOLINT(readability-suspicious-call-argument) */
    if(p->bt_nkeys) {
        struct scc_btnode_base **plinks = scc_btnode_links(base, p);
...
//?     struct scc_btree_base *restrict base, struct scc_btnode_base *restrict curr, \
//?     struct scc_btnode_base **nodes, size_t *bounds, size_t elemsize)
//?
//?     Traverse the tree back towards to root, balancing as needed
//?
//?     .. note::
//?
//?         Internal use only
//?
//?     :param base: B-tree base address
//?     :param curr: Leaf node in which the traversal is started
//?     :param nodes: Stack of nodes traversed while finding leaf
//?     :param bounds: Stack of bounds computed while finding leaf.
//?                    The nth bound in the stack is the index of the
//?                    link in the nth node to obtain the n+1th node
//?     :param elemsize: Size of the elements in the tree
...
static _Bool scc_btree_remove_non_preemptive(struct scc_btree_base *restrict base, void *restrict btree, size_t elemsize) {
    ...
    while(1) {
        ...
        if(!found && bound < curr->bt_nkeys && !base->bt_compare(scc_btnode_value(base, curr, bound, elemsize), btree)) {
            found = curr;
            fbound = bound;
